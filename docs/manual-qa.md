...
- [ ] 单行内选中一段文字：高亮严格落在该行行框内，上下相邻行不出现任何色块（Typora 行为）；在标题行下方、表格/代码块/图片下方各试一次，高亮不随文档位置向上漂移
...
- [ ] 选区几何对设置免疫：把 Settings 里字号在 12/16/24、行高在 1.4/1.6/2.0 之间切换，逐一复查上面两条仍然成立（行框吸附不依赖 `defaultLineHeight`）
