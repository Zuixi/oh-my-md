...
- [ ] 点击数学块：块正下方弹出源码编辑框（`omd-math-popup`），逐键输入实时更新上方 KaTeX 渲染且块 DOM 不重建/不闪烁；Escape 或失焦（点到编辑框外）关闭编辑框；再次点击块把焦点还给编辑框；键盘把光标移入块内仍显示 `$$` 源码、编辑框随块卸载消失；只读文档点击不弹编辑框（桌面端只读场景如适用）；在多行 TeX 的开头/中间/结尾附近点击并输入，确认插入发生在点击的光标位置；用鼠标拖选后输入，确认选区被替换
