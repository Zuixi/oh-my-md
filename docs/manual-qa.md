...
- [ ] dark 选区可读：拖选正文/标题/行内 code，选区底为半透明深青、文字清晰（对比度 ≥4.5:1），无浅紫底浅字
...
- [ ] 围栏/缩进代码块整行等宽底色；块内 `**` 等行内语法不折叠；高亮属 M2。正在输入的未闭合围栏行（光标在其上）保持纯文本不显灰底；按 Enter 后自动补全闭合围栏成完整块、光标落在内容行内闪烁（` ``` ` 与 ` ```lang `、`~~~`、更长反引号 run 同样成立）；引用/列表内的围栏行 Enter 不劫持
...
- [ ] **代码块编辑态观感（Typora）**：点击进入后 header（标题/语言）仍在块顶、内容行带行号、容器保持圆角边框、``` 围栏不可见；在编辑态改标题或切换语言会写回 fence info；把光标移到块尾围栏行（↑ 到最后一行再 ↑）时尾围栏以裸文本出现可编辑；编辑态代码有语法高亮（明暗两主题）；mermaid 块编辑态保持朴素源码行（无 chrome）
...
- [ ] 代码块：灰底容器（`#f8f8f8` / 暗色等价）、紧凑行号 gutter（无双重左距、行高约 1.45）、Shiki 高亮；顶栏默认显示（标签/fence info、语言搜索下拉可点选、Copy 图标，成功后短暂 Copied）；↑/↓ 进入块内卸载 widget、源码行可编辑且光标闪烁，无报错/卡死；光标所在源码行同样保持灰底（不被当前行透明规则抹掉，块读感连续）；标签与语言写入 fence info 第一 token + 后缀；⌘E Source 显完整 ```
