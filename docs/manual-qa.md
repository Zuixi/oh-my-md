...
- [ ] 打开 10–50MB 样本：打开前出现确认框（「以源码模式打开」），确认后全程可见 loading + 可取消（取消按钮与 Escape 等效；模态/面板打开时 Escape 优先关闭它们）；进入源码安全模式。
- [ ] 打开 >50MB 样本：确认框默认只读纯文本；接受后编辑器只读（无法输入）、横幅提示建议拆分；取消则不读取。
- [ ] 长行文件（<50k 行但 >10MB，如超长段落）：同样进入安全模式（字节轴兜底）。
