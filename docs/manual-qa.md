...
- [ ] 代码块：灰底容器（`#f8f8f8` / 暗色等价）、紧凑行号 gutter（无双重左距、行高约 1.45）、Shiki 高亮；hover 显示顶栏（标签/fence info、语言搜索下拉可点选、Copy 图标，成功后短暂 Copied）；↑/↓ 进入块内卸载 widget、源码行可编辑且光标闪烁，无报错/卡死；标签与语言写入 fence info 第一 token + 后缀；⌘E Source 显完整 ```
