...
- [ ] 在 `**粗体**` 中间打字：语法标记保持折叠（路线 A：装饰标记不随光标展开，点击/移动光标不闪源码）；增删格式走快捷键/命令面板
...
- [ ] 点击数学块：块正下方弹出源码编辑框（`omd-math-popup`），逐键输入实时更新上方 KaTeX 渲染且块 DOM 不重建/不闪烁；弹窗编辑器为 CodeMirror：LaTeX 语法高亮可见（命令/注释/括号着色，深浅主题各正确）；弹窗内 Cmd+Z 撤销上一步并写回文档；Tab 插入两个空格、不触发焦点导航关闭弹窗；中文 IME 组词期间外部变更（如撤销/自动保存回写）不打断预输入；Escape 或失焦（点到编辑框外）关闭编辑框；再次点击块把焦点还给编辑框；键盘把光标移入块内仍显示 `$$` 源码、编辑框随块卸载消失；只读文档点击不弹编辑框（桌面端只读场景如适用）；在多行 TeX 的开头/中间/结尾附近点击并输入，确认插入发生在点击的光标位置；用鼠标拖选后输入，确认选区被替换；用户在设置里选定的等宽编辑器字体同样作用于弹窗编辑器
...
- [ ] 点击标题行选中文字不连带高亮下一行；拖选扫过 `**粗体**`/链接时保持渲染形态；单击加粗/标题/列表文本只定位光标，不显 `**`/`#`/`-` 源码（链接/图片/行内公式所在行光标进入仍展开，属刻意保留）
...
- [ ] dark 选区可读：拖选正文/标题/行内 code，选区底为不透明深蓝（#264f78，VS Code Dark+ 同源）且文字清晰（对比度 ≥4.5:1）；编辑器失焦（点其他窗口）后选区转为暗灰 #3a3d41，仍可辨认、不再与背景融为一体，重新聚焦恢复深蓝
- [ ] dark 光标可见：正文/空行/行内 code/代码块内的光标为 2px 纯白竖线，空行处不隐身；失焦后光标隐藏（VS Code/Typora 行为），重新聚焦恢复闪烁
- [ ] 嵌套编辑器 caret 同色：数学块弹窗（CodeMirror）与代码块标题输入框聚焦后，其原生 caret 为主题光标色（dark 下纯白，非浏览器默认黑；light 下 #1a1a1a）
- [ ] 两主题下选区颜色一致：block widget（code block、table）内拖选与正文同为 `--omd-selection-bg`，无原生蓝色；点击 table 单元格进入编辑后选字同样为主题色
- [ ] 整块覆盖选中（选区完整包含 mermaid/table 等块）覆盖层颜色与文字选区同色，无双重叠色明显加深
...
- [ ] 会话恢复的文件夹已不存在（移动/删除/换机后启动）：不进入死工作区状态，仅提示一次「上次的工作区文件夹已不可用」并新建草稿；文件树不残留失效路径，后续无重复报错
- [ ] 文件树某目录列举持续失败（如权限被收回）：同一错误只 toast 一次；错误内容变化后会再次提示；目录恢复可列举后不再报错
...
## 渲染（对照 styles.css 类名）
- [ ] 文档末尾的最后一行可滚动到视口大约中部，且仍可点击、编辑；不会插入源码行或空白 Markdown
- [ ] 宽窗口中，居中的可读列可达到 900px；窄窗口不会产生页面水平溢出
- [ ] 空行密度：非光标空行渲染约半行高、连续空行自然叠加（Typora 观感）；光标进入某空行该行恢复全高、离开后重新折叠；拖选跨空行时空行保持半高不展开；点击半高空行仍能把光标放入该行
- [ ] 各级标题字号正确；ATX 的 `#` 与 Setext 的 `===` / `---` 始终折叠（光标进入也不展开）
- [ ] 加粗 / 斜体 / 删除线 样式正确，语法标记被折叠；CJK 紧邻的 `__粗体__` / `_斜体_` 与 `**` / `*` 等效
- [ ] 行内代码 背景等宽字体
- [ ] `==高亮==` 与 `<mark>高亮</mark>` 黄底，`<u>下划线</u>` 下划线，标记始终折叠
- [ ] 链接 蓝色，URL 折叠，光标进入展开
- [ ] 单击 `[^id]` 跳到对应定义并滚入视口；再单击该定义的 `[^id]:` 标记回到刚才的引用；缺失定义时不跳转、不报错
- [ ] 单击相对路径 `.md` / `.markdown` 链接（可带 `#anchor`）打开该文件标签；文件不存在时报错且不新建；`https://` / `mailto:` 交给系统默认应用打开（Windows 重点验证 Ctrl+Click 能启动默认浏览器）；图片等其它相对路径单击无动作
- [ ] 任务复选框可点切换，状态写回源码（`[ ]`⇄`[x]`）
- [ ] 无序列表 marker 折叠为 `•`，有序列表按项序显示 1,2,3，并把源码改成连续编号（光标进入该行与预览一致）；任务项不叠 bullet
- [ ] 嵌套列表逐级缩进，换行的续行对齐到正文（悬挂缩进）；光标进入该行时源码缩进显露
- [ ] 围栏/缩进代码块整行等宽底色；块内 `**` 等行内语法不折叠；高亮属 M2。正在输入的未闭合围栏行（光标在其上）保持纯文本不显灰底；按 Enter 后自动补全闭合围栏成完整块、光标落在内容行内闪烁，立即在渲染好的编辑态（header + 行号）内继续输入（` ``` ` 与 ` ```lang `、`~~~`、更长反引号 run 同样成立）；文档**中间**（下方有正文/其他块）同样立即补全成空块，下方内容原位留在块之后；已完整闭合的块在围栏行按 Enter 不劫持（普通换行）；引用/列表内的围栏行 Enter 不劫持
...
## M2 块渲染
- [ ] **点击不闪源码**：Live 下点击含链接/图片/行内公式段落的普通文本，整段保持渲染（不裸奔 `[text](url)`）；点击链接文字本身才展开该链接语法，光标离开即折回；脚注引用/定义标记同理（光标进入才展开）
- [ ] **表格点击不闪**：单击表格的单元格就地编辑；单击 wrap 空白区/边框/工具栏空隙保持渲染不翻源码；点右上角 `</>` 按钮才显式进源码；↑/↓ 键盘进块仍显源码
- [ ] **代码块点击进编辑**：左键点击代码体，光标落在被点击的源码行并闪烁可编辑；拖动选择代码文本不进源码（保持渲染 + 原生选择）；点击顶栏（标题/语言/复制）不进源码；只读文档点击代码体保持渲染
- [ ] **代码块编辑态观感（Typora）**：点击进入后 header（标题/语言）仍在块顶、内容行带行号、容器保持圆角边框、``` 围栏不可见；在编辑态改标题或切换语言会写回 fence info；把光标移到块尾围栏行（↑ 到最后一行再 ↑）时尾围栏以裸文本出现可编辑；编辑态代码有语法高亮（明暗两主题）；mermaid 块编辑态保持朴素源码行（无 chrome）；删除尾 ``` 后的未闭合块：内容行照常编号、最后一行代码不被折叠吞掉
- [ ] **Mermaid 进源码**：点击 mermaid 块或其 `</>` 按钮、↑/↓ 进入，卸载为源码行可编辑；光标离开恢复图表
- [ ] 表格渲染为 HTML 表格，对齐正确；单击单元格就地编辑源码，文字不被整段蓝色全选、不出现输入框边框/焦点框，列宽和行高不抖动（Enter/Tab 提交并移到下一格，Shift-Tab 上一格，Escape 取消）；右上角可在下方插行、右侧插列、删当前行/列（图标按钮，悬停有 title）
- [ ] 表格编辑源码忠实：转义管道单元格（`a\|b` 显示为 `a|b`、源码不改写）与 ragged 行表格（某行缺列尾）均正常渲染；编辑其中一个真实单元格后，其它单元格与分隔行的源码字节、间距、单边外管风格不变（改一处不重排全表）
- [ ] 表格键盘连续编辑：Enter 提交并移到下一格、Shift-Tab 上一格、Escape 取消；Tab 从最后一格离开会在表尾插入空行并聚焦新行首格（Enter 在末格只提交不插行，Shift-Tab 在首格只提交不开表外输入框）；在 20×10 表格里连续 Tab 逐格走一遍不卡顿、不落错格、不残留半编辑状态
- [ ] 表格工具栏边界：仅剩一行时「−row」禁用、仅剩一列时「−col」禁用、只读文档全部编辑控件禁用；禁用态不改变表格布局
- [ ] 表格编辑高亮不改变尺寸：打开单元格时所在行/列出现浅色高亮（omd-table-row-active / omd-table-col-active），列宽、行高、表格整体尺寸不因高亮抖动或重排
- [ ] 多标签表格互不干扰：两个标签各含一个表格，在其中一个标签的表格里 Tab 连续编辑时，另一个标签不弹出输入框、不被抢焦点
- [ ] 引用/列表内表格的行列操作保留前缀：blockquote（`> `）或列表缩进内的表格执行插行/插列/删行/删列后，新增与改动的行仍带正确前缀（`> `/缩进），外层块结构与对齐不破坏
- [ ] 表格单元格内 Markdown 任意语法渲染：粗体/斜体/删除线/高亮/下划线/行内代码/行内公式/链接/autolink/图片/emoji/`<br>`，以及单行列表（`- item`）、引用（`> q`）、代码块（`` ``` ``）；导出 HTML 与预览一致
- [ ] 代码块：灰底容器（`#f8f8f8` / 暗色等价）、紧凑行号 gutter（空行保留整行高度，行号连续不断档）（无双重左距、行高约 1.45）、Shiki 高亮；顶栏默认显示（标签/fence info、语言搜索下拉可点选、Copy 图标，成功后短暂 Copied）；↑/↓ 进入块内卸载 widget、源码行可编辑且光标闪烁，无报错/卡死；光标所在源码行同样保持灰底（不被当前行透明规则抹掉，块读感连续）；标签与语言写入 fence info 第一 token + 后缀；⌘E Source 显完整 ```
...
- [ ] 选择 JetBrains Mono 或另一款外观明显不同的已安装字体后，正文、行内代码、源码样式代码行、渲染后的 Shiki 代码块和数学弹窗 textarea 都使用该字体族
...
## 多窗口（Multi-window，2026-09）

以下矩阵需 `pnpm dev` 双窗口交互（部分项需 macOS/Windows 实机或打包产物，如标注）；本环境未执行 GUI，**全部条目待人工验证**（保持未勾选）。

- [ ] **New Window 入口**：File 菜单「New Window」、快捷键 ⇧⌘N / Ctrl+Shift+N、命令面板、应用内菜单四处都能打开新编辑器窗口；新窗口为空白未命名草稿、默认 800×600
- [ ] **第二独立工作区**：新窗口可单独 Open Folder / 打开文件，与主窗口互为独立工作区（各自文件树、标签、撤销栈、查找条）
- [ ] **窗口隔离**：两个窗口各开不同文件：⌘O/⌘S/⌘W/⌘F/格式化命令只作用于当前聚焦窗口；一个窗口切 live/source、主题、专注模式不影响另一窗口的编辑器状态
- [ ] **关闭单窗口只 flush 该窗口**：窗口 A 有 1s 防抖未到期的编辑时关掉窗口 B → A 毫发无损（内容、dirty、防抖节奏不变）；被关闭的窗口不再出现在下次会话恢复中，A 恢复正常
- [ ] **退出 flush 全部窗口**：⌘Q（macOS）/ 应用内 Quit / 红X 关最后一个窗口三条路径 → 每个存活窗口的会话均落盘；重启后全部恢复
- [ ] **更新重启 flush 全部窗口**：多窗口下点 Restart and install → 所有窗口完成 session flush 后才退出安装；任一窗口超时不 ack 则中止安装、应用保持运行
- [ ] **OS 打开已打开的文件**：双击 / Open With 一个已在某窗口打开的 .md → 聚焦该窗口并激活对应标签，不重复开标签
- [ ] **OS 打开新文件按评分落窗**：目标在某窗口打开的文件夹内（+5）优先；与某窗口已打开文件同目录（+1）次之；无任何信号落最近聚焦（MRU）窗口；全部窗口关闭时新开窗口并携带该文件
- [ ] **重启恢复所有窗口**：上次退出时的每个窗口逐一恢复，含位置/大小/最大化状态（按保存的物理像素几何精确还原）及各自标签内容与文件夹（每窗口恢复自己 shard 的标签/工作区，非主窗口不得回退为空白草稿）；被关闭过的窗口不恢复
- [ ] **macOS**：关掉最后一个窗口应用驻留（Dock 图标仍在、菜单栏可用），点 Dock 图标重开一个窗口
- [ ] **Windows**：关掉最后一个窗口应用完全退出（无残留进程；下次启动为全新会话）
