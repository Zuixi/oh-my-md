...
- [ ] 代码块：灰底容器（`#f8f8f8` / 暗色等价）、紧凑行号 gutter（无双重左距、行高约 1.45）、Shiki 高亮；hover 显示顶栏（标签/fence info、语言搜索下拉向下展开、Copy）；↑/↓ 进入块内可编辑且顶栏仍仅 hover 时出现；标签与语言写入 fence info；⌘E Source 显完整 ```
