...
The reveal policy (Route A): **a non-empty selection never reveals anything; decorative marks never reveal even to a collapsed caret; the caret reveals only link/image/math/footnote lines and block sources**:

- `nearCursor` returns false for any non-empty selection (no line reveal while dragging) and is used only by the link/image/math/footnote reveal paths.
- `cursorInside` is collapsed-caret-only (`sf === st && sf >= from && sf < to`); the old non-empty overlap branch and its triple-click special case are gone. It now serves QuoteMark, fence marks, entities/emoji, and links/images inside quotes.
