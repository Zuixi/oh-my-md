...
| Rust workspace search, 1,000 files × 200 lines | `79.90 ms` | Post-fix verification printed `workspace search 1000x200: 79.90ms`; the deterministic capped-search algorithm still returned `MAX_SEARCH_HITS` and `truncated`, preserving the 500-hit cap and completion-order-independent first paths. Earlier runs used the pre-fix collection algorithm, so their `42.10 ms` and `37.12 ms` values are retained only as historical context; timings are advisory and load-dependent. |
| Full repository gate | `pnpm verify` passed | Engine: `43` files / `441` tests; desktop: `73` files / `584` tests; Rust: `144` passed / `1` ignored; Vite transformed `4243` modules and built in `4.50s`; the Rust app binary linked successfully. |
