...
- `pnpm --filter @omd/engine bench` 跑 advisory 大文档基准（typing p95/冷解析/装饰重建/字数统计/切Live种子/open-live摄入/安全模式窗口化typing/表格滚动进入与高度估算增益）；预算超限只告警不阻断，CI 中 continue-on-error。
