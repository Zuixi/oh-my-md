...
- **`view.documentTop` is the only valid origin for `BlockInfo` coordinates.** `BlockInfo.top`/`.bottom`/`.height` are measured from the top of the first line — *below* `.cm-content`'s `padding-top` — while `contentRect.top` is the border box. Mixing them offsets every block-derived y by the padding (16px against a ~25.6px line box), which is what made single-line selections paint across three rows. Equally, `defaultLineHeight` is a default, not a row pitch: headings are 1.25–1.8em and block widgets are arbitrary, so never synthesize a document-wide line grid — read each block's real box instead. Both rules are enforced by the vendoring guards in `test/tightSelection.test.ts`, one of which also fails when the installed `@codemirror/view` version drifts from the version recorded in the vendored file header (the prompt to re-diff, since upstream fixes to `drawSelection.ts` no longer reach us automatically).
