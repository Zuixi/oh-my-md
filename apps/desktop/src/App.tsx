...
export default function App({
  ...
  const commands: AppCommand[] = [
    { id: "open", label: t("cmd.label.open"), shortcut: "⌘O", run: () => void openFile() },
    { id: "save", label: t("cmd.label.save"), shortcut: "⌘S", run: () => void saveFile(workspaceRef.current.activeId, "explicit") },
    { id: "save-as", label: t("cmd.label.save-as"), shortcut: "⇧⌘S", run: () => void saveFile(workspaceRef.current.activeId, "explicit", true) },
    { id: "folder", label: t("cmd.label.folder"), run: () => void chooseFolder() },
    { id: "tab", label: t("cmd.label.tab"), shortcut: "⌘N", run: newTab },
    { id: "close", label: t("cmd.label.close"), shortcut: "⌘W", run: () => requestCloseTab(workspaceRef.current.activeId) },
    ...
    { id: "preferences", label: t("cmd.label.preferences"), shortcut: "⌘,", run: () => setSettingsOpen(true) },
    { id: "sidebar", label: t("cmd.label.sidebar"), shortcut: "⌘\\", run: () => setSidebarOpen(open => !open) },
    { id: "outline", label: t("cmd.label.outline"), shortcut: "⇧⌘O", run: () => setOutlineOpen(open => !open) },
    { id: "typewriter", label: t("cmd.label.typewriter"), run: () => setTypewriter(on => !on) },
    { id: "source", label: t("cmd.label.source"), shortcut: "⌘E", run: () => {
      const view = viewRef.current
      if (view) try { view.dispatch(applyToggle(view.state)) } catch { /* mock views */ }
    } },
    { id: "bold", label: t("cmd.label.bold"), shortcut: "⌘B", run: runFormat(toggleBold) },
    { id: "italic", label: t("cmd.label.italic"), shortcut: "⌘I", run: runFormat(toggleItalic) },
    { id: "strikethrough", label: t("cmd.label.strikethrough"), shortcut: "⇧⌘X", run: runFormat(toggleStrikethrough) },
    { id: "inline-code", label: t("cmd.label.inline-code"), shortcut: "⇧⌘`", run: runFormat(toggleInlineCode) },
    { id: "code-block", label: t("cmd.label.code-block"), shortcut: "⇧⌘K", run: runFormat(toggleCodeBlock) },
    { id: "heading-1", label: t("cmd.label.heading-1"), shortcut: "⌘1", run: runFormat(toggleHeading(1)) },
    { id: "heading-2", label: t("cmd.label.heading-2"), shortcut: "⌘2", run: runFormat(toggleHeading(2)) },
    { id: "heading-3", label: t("cmd.label.heading-3"), shortcut: "⌘3", run: runFormat(toggleHeading(3)) },
    { id: "heading-4", label: t("cmd.label.heading-4"), shortcut: "⌘4", run: runFormat(toggleHeading(4)) },
    { id: "heading-5", label: t("cmd.label.heading-5"), shortcut: "⌘5", run: runFormat(toggleHeading(5)) },
    { id: "heading-6", label: t("cmd.label.heading-6"), shortcut: "⌘6", run: runFormat(toggleHeading(6)) },
    { id: "ordered-list", label: t("cmd.label.ordered-list"), shortcut: "⌥⌘7", run: runFormat(toggleOrderedList) },
    { id: "unordered-list", label: t("cmd.label.unordered-list"), shortcut: "⌥⌘8", run: runFormat(toggleUnorderedList) },
    { id: "blockquote", label: t("cmd.label.blockquote"), shortcut: "⌥⌘9", run: runFormat(toggleBlockquote) },
    { id: "link", label: t("cmd.label.link"), shortcut: "⌘K", run: runFormat(insertLink) },
    { id: "insert-image", label: t("cmd.label.insert-image"), run: insertImage },
    { id: "find", label: t("cmd.label.find"), shortcut: "⌘F", run: () => {
      setFindOpen(true)
      setReplaceOpen(false)
    } },
    { id: "search", label: t("cmd.label.search"), shortcut: "⇧⌘F", run: () => setSearchOpen(true) },
