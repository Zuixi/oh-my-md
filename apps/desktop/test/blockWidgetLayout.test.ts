...
describe("block widget layout", () => {
  ...
  it("assembles the editing-state code container and line numbers", () => {
    ...
    // 渲染态与编辑态几何对齐（消除点击抖动）：字号 0.9em，行高 1.45，padding-left 2.5em
    expect(num).toMatch(/font-size:\s*0\.9em/)
    expect(num).toMatch(/line-height:\s*1\.45/)
    expect(num).toMatch(/padding-left:\s*2\.5em/)
    ...
    expect(first).toMatch(/padding-top:\s*6px/)
    ...
    expect(last).toMatch(/padding-bottom:\s*10px/)
