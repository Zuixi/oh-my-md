...
10. **Route A mark folding** (`decorations/types.ts` documents the split): paired emphasis marks, ATX/Setext heading marks, and list marks fold **unconditionally** — the caret never reveals them, so clicking text only places the caret and lines never reflow. Editing goes through `format/commands.ts` toggles, or Backspace at a folded boundary (mid-line atoms delete whole via `skipAtomic`; line-start marks delete one char = progressive demotion). Syntax whose source is the sole editing entry (link URL/title, image, inline math, footnote reference/definition) reveals via **node-level `cursorInside`** — the caret must enter that syntax's own span, never just its line (line-level reveal made any click in a soft-wrapped paragraph unfold every link on it). Gate at the whole node, never per-child: the caret in link text is inside the LinkMark span but outside the URL span, and per-child gating yields a half-revealed `[text](` state. `cursorInside` also serves QuoteMark, fence marks, and entities/emoji; line-based `nearCursor` survives only for blank-line density (build.ts), whose unit is a line by definition. Never reintroduce caret-driven reveal for decorative marks — that is the click-flash regression this model removed.
11. **Blank-line density** (Typora look): `collectDecorationSpecs` scans its range line-by-line and emits `line:omd-empty` for whitespace-only lines the caret is not on (desktop CSS shrinks them to ~half height). Blank lines have no syntax nodes — this scan lives outside the tree iteration. The caret's own blank line stays full-height (caret needs a line box), non-empty selections never expand one (`nearCursor` semantics — mid-drag height flips move `posAtCoords` endpoints, atomicRanges Rule 3 family). The spec's `from` must be inside the queried range so non-line-aligned chunk boundaries don't double-emit. Never hide blank lines with `display:none` — CM's line measurement and click mapping need a real box. Blank lines inside a block widget's replaced range are filtered like any other covered decoration.
