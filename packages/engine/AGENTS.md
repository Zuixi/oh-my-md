...
7a. `MathBlockWidget` reuse/refresh contract — 逐键回写文档不得重建 widget DOM: its `eq` compares `src` + `embed` (with an `instanceof` guard against cross-type pairing). CodeMirror's findWidget pass 0 reuses DOM of tiles whose `eq` succeeds WITHOUT calling `updateDOM`; `updateDOM` only runs (pass 1, matched by constructor) when `eq` FAILED. So `eq` must return false whenever the content changed, and `updateDOM` is the in-place refresh path: it re-syncs the popup textarea (no focus guard — Undo/Redo and external edits must re-sync too; during normal typing the write-back makes the sync a no-op) and re-renders the KaTeX preview via the rAF-coalesced `throwOnError: false` render. The popup's `ResizeObserver` is stored on the widget and disconnected on Escape/blur close and in `destroy()`. Editing happens in a click-opened popup (`omd-math-popup` / `omd-math-editor` inside the widget wrap): `enterSourceOnClick` returns `false`, `onWrapClick` opens/refocuses the textarea, each input re-reads the block text from the **current document** via `blockWidgetRange` (the wrap listener's `this.src` is stale after the first write-back) and dispatches a same-position replace; the selection stays outside the block so the widget stays mounted. Escape/blur (outside the popup) close it; read-only views never open it.
