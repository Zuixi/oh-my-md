...
5. Expose desktop-facing behavior through `src/index.ts`: `editorExtensions(options)`, plus read-only `collectOutline(state)` and `exportHtml(state)`, plus the ordered-list normalization commands (`getPendingOrderedListNormalization`, `acceptOrderedListNormalization`, `rejectOrderedListNormalization`), plus navigation lookups (`classifyLink`, `footnoteAt`, `footnoteDefinitionPosition`, `footnoteReferencePosition`), plus table range edits (`replaceTableCell`, `insertTableRow`, `insertTableColumn`, `deleteTableRow`, `deleteTableColumn`, `escapeTableCellValue` from `tables/edit.ts`). Each table helper is pure: it takes the table source plus table-relative model data and returns `readonly TableSourceChange[]` range edits (`from`/`to`/`insert`, table-relative) or `null` for stale/out-of-range input — never a serialized whole-table string. Do not make desktop consumers assemble engine internals or re-parse Markdown. Command helpers stay pure: ordered-list normalization returns a `TransactionSpec` (or a `"stale"` result) and table edits return range changes; neither dispatches. Read pending and build accept/reject transactions in the same tick as the triggering update; stale offsets if deferred.
